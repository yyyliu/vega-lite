...
export function predicate(model: Model, name: string, type: SelectionTypes, resolve?: string, datum?: string, parent?: string): string {
  const store = stringValue(name + STORE),
        op = PREDICATES_OPS[resolve || 'global'];
  datum = datum || 'datum';
  parent = parent === null ? null : stringValue(model.getName(''));
